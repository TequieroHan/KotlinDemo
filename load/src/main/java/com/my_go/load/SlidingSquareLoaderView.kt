...
import com.my_go.load.extensions.getLeftMargin
import com.my_go.load.extensions.onEnd
import com.my_go.load.extensions.setLeftMargin
import com.my_go.load.utils.Constant
import com.my_go.load.utils.way.WayCreator
import com.my_go.load.utils.way.WayHolder
import com.my_go.load.utils.way.WayType
import com.my_go.load.views.SquareView
import java.util.ArrayList

/**
 * Create by Package com.my_go.load
...
class SlidingSquareLoaderView : RelativeLayout, StateController {
    ...
    private fun prepareDefaultPositions() {
        viewsPosition = resources.getIntArray(R.array.view_positions)
    ...
    private fun prepareAttributesSet(attrs: AttributeSet?) {
        ...
        viewMargin = typedArray.getDimensionPixelSize(
            R.styleable.SlidingSquareLoaderView_sslv_margin,
            resources.getDimensionPixelSize(R.dimen.dp_5)
        )
        viewCornerRadius = typedArray.getDimensionPixelSize(
            R.styleable.SlidingSquareLoaderView_sslv_margin,
            resources.getDimensionPixelSize(R.dimen.dp_5)
        )
        viewStartGradientColor = typedArray.getColor(
            R.styleable.SlidingSquareLoaderView_sslv_start_gradient_color,
            ContextCompat.getColor(context, R.color.color_ff870b)
        )
        viewEndGradientColor = typedArray.getColor(
            R.styleable.SlidingSquareLoaderView_sslv_end_gradient_color,
            ContextCompat.getColor(context, R.color.color_ff0662)
        )
        parentBackground = typedArray.getColor(
            R.styleable.SlidingSquareLoaderView_sslv_background_color,
            ContextCompat.getColor(context, R.color.color_4b4b4b)
        )
        squareViewSize = typedArray.getDimensionPixelSize(
            R.styleable.SlidingSquareLoaderView_sslv_square_size,
            resources.getDimensionPixelSize(R.dimen.dp_35)
        )
        typedArray.recycle()
    }
...
//                visibility = View.GONE
    ...
    private fun prepareSquareViews(): SquareView {
        ...
        view.prepareGradientDrawable(
            viewStartGradientColor,
            viewEndGradientColor,
            viewCornerRadius,
            squareViewSize
        )
